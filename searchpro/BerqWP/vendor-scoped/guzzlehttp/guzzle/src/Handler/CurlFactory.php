...
namespace BerqWP\GuzzleHttp\Handler;

use BerqWP\GuzzleHttp\Exception\ConnectException;
use BerqWP\GuzzleHttp\Exception\RequestException;
use BerqWP\GuzzleHttp\Promise as P;
use BerqWP\GuzzleHttp\Promise\FulfilledPromise;
use BerqWP\GuzzleHttp\Promise\PromiseInterface;
use BerqWP\GuzzleHttp\Psr7\LazyOpenStream;
use BerqWP\GuzzleHttp\TransferStats;
use BerqWP\GuzzleHttp\Utils;
use BerqWP\Psr\Http\Message\RequestInterface;
use BerqWP\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx) : PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \BerqWP\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri) : string
    {
        ...
        $redactedUriString = \BerqWP\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \BerqWP\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \BerqWP\GuzzleHttp\Psr7\Utils::streamFor($sink);
